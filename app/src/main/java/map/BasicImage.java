...
public class BasicImage {
   ...
   public void setColor(int[] xy, Color c) {
     ...
     * @param extra Extension-term of overlay.
     *
     * @pre l fits inside this image with some margin.
     */
    public void drawLabelOverlay(LabelLayout l, Color c, double extra) {
        Graphics2D g = createGraphics();
        g.setColor(c);

        if (l.getNoRows() > 1) {
            double[] bs = Math2.extendBounds(l.getBounds(), extra);
            double strokeHeight = bs[3] - bs[1];
            g.setStroke(new BasicStroke((float) strokeHeight,
                                        BasicStroke.CAP_ROUND,
                                        BasicStroke.JOIN_MITER));
            int[] bsi = Math2.toIntBounds(bs);
            int ymid = Math2.toInt((bs[1] + bs[3]) / 2);
            g.drawLine(bsi[0], ymid, bsi[2], ymid);
            return;
        }

        double strokeHeight = l.getTallestBoxHeight() + extra*2;
        g.setStroke(new BasicStroke((float) strokeHeight,
                                    BasicStroke.CAP_ROUND,
                                    BasicStroke.JOIN_MITER));

        Box[] row = l.getBoxes();
        int[] cursor = Math2.toInt(row[0].getLeftMid());
        cursor = Math2.step(cursor, row[ row.length-1 ].getDirVector(), -extra);
        ...
        int[] end = Math2.step(new int[]{cursor[0], cursor[1]}, row[ row.length-1 ].getDirVector(), extra);
        g.drawLine(cursor[0], cursor[1], end[0], end[1]);
    }
    ...
    public void drawBounds(int[] bs) {
        ...
        g.drawLine(bs[0], bs[3], bs[0], bs[1]);
    }
    ...
    /**
     * Draw img on this image, at top-left tl.
     */
